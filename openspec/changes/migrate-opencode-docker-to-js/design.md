...
The active wrapper behavior still lives in Bash, while `bin/opencode-docker.js` is the published entry. The migration must make JS authoritative for in-scope flows without regressing current Unix-like behavior.

# Goals

- Make `bin/opencode-docker.js` authoritative for core flows on Linux, macOS, and Windows.
...
- `bin/opencode-docker.js` remains a thin executable adapter that delegates to modules under `./src/`.
