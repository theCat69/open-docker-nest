...
- [x] 1.1 Update `openspec/specs/dockerized-opencode-workflow/spec.md` to include delta requirements for cache-ctrl image installation and la-briguade integration behavior.
...
- [x] 4.2 Run command parity smoke checks through `bin/opencode-docker` using workflow-spec slash-command form (`opencode --help`, `opencode run "/opsx-propose demo-change"`, `opencode run "/opsx-explore demo-change"`, `opencode run "/opsx-apply demo-change"`, `opencode run "/opsx-archive demo-change"`).
