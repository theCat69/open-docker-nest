...
- [x] 1.1 Update the `--shell` execution path so `bin/opencode-docker --shell` opens an interactive shell in the mounted project context as user `opencode`.
- [x] 1.2 Ensure shell mode uses `/home/opencode` as the runtime home directory.
- [x] 1.3 Keep the shell-mode fix isolated to user-selection behavior so `bin/opencode-docker` still defaults to `opencode` when no command is provided.
- [x] 1.4 Preserve direct command pass-through behavior so `bin/opencode-docker -- <command> ...args` is forwarded unchanged and does not inherit shell-mode behavior.
...
- [x] 3.1 Add or update smoke validation for `bin/opencode-docker --shell` to assert the interactive session runs as user `opencode`.
- [x] 3.2 Add or update smoke validation for `bin/opencode-docker --shell` to assert the runtime home directory is `/home/opencode`.
- [x] 3.3 Verify `bin/opencode-docker` with no command arguments still executes the default `opencode` command and does not implicitly start a shell.
- [x] 3.4 Verify `bin/opencode-docker -- <command> ...args` still passes the provided command and arguments through unchanged.
